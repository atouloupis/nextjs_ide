...
export default function UserIDE({ userName, accessToken }: UserIDEProps) {
  ...
  // Initialize xterm.js terminal when showIDE is true
  useEffect(() => {
    ...
    if (showIDE && terminalRef.current) {
      import("xterm").then(({ Terminal }) => {
        ...
        // Auto-scroll after rendering new content
        term.onRender(() => {
          term?.scrollToBottom();
        });
  ...
  // Load saved code from localStorage (per user + language)
  useEffect(() => {
    ...
    <div className="mt-4">
      ...
      {showIDE && (
        <div className="flex flex-col gap-2">
          ...
          <div className="h-[400px] border rounded bg-black text-white overflow-hidden relative">
            <div ref={terminalRef} 
                className="absolute inset-0 overflow-hidden" 
                style={
                  {
                    // Forcer la div xterm-screen à respecter les dimensions du parent
